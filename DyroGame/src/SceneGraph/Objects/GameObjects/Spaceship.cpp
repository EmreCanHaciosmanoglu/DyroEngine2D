...
SpaceShip::SpaceShip(const Vector2D& startPosition)
...
{
	input->bindInput(InputBinding('W', std::bind(&SpaceshipPlayerControllerComponent::moveForward, this->controller), InputStateType::DOWN));
	input->bindInput(InputBinding('A', std::bind(&SpaceshipPlayerControllerComponent::rotateLeft, this->controller), InputStateType::DOWN));
	input->bindInput(InputBinding('S', std::bind(&SpaceshipPlayerControllerComponent::moveBackward, this->controller), InputStateType::DOWN));
	input->bindInput(InputBinding('D', std::bind(&SpaceshipPlayerControllerComponent::rotateRight, this->controller), InputStateType::DOWN));

	input->bindInput(InputBinding(VK_SPACE, std::bind(&SpaceshipPlayerControllerComponent::startFire, this->controller), InputStateType::DOWN));
	input->bindInput(InputBinding(VK_SPACE, std::bind(&SpaceshipPlayerControllerComponent::stopFire, this->controller), InputStateType::RELEASD));
}